...
use crate::content_reader::read_content_model;
...
use crate::project_scan::{ProjectSnapshot, scan_project};
use crate::site_generator::generate_site_files;
use crate::state_diff::{StateDiff, diff_states};
...
pub fn assemble_project(project_root: &Path) -> io::Result<AssemblyReport> {
    ...
    let snapshot = scan_project(project_root, &config)?;
    ...
    let generated_files = generate_site_files(&content_model, &block_dataset)?;
...
mod tests {
    ...
    use super::*;
    use crate::project_scan::ProjectFile;

    #[test]
    fn rejects_generated_source_inside_authored_src() {
        let snapshot = ProjectSnapshot {
            project_root: PathBuf::from("/tmp/example"),
            files: vec![ProjectFile {
                relative_path: "src/generated/site.ts".to_string(),
                size: 10,
                hash: "hash".to_string(),
            }],
        };

        let error = validate_source_boundaries(&snapshot).unwrap_err();

        assert!(
            error
                .to_string()
                .contains("src/generated/site.ts is forbidden")
        );
    }
}
